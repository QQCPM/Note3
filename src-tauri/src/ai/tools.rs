...
pub fn get_database_tools() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            description: "Add a new row to the database with complete data for ALL columns. REQUIRED: You MUST include row_data object with values for EVERY column in the table. Use your built-in knowledge for well-known facts (planets, countries, etc.) - web search is optional. Call this ONCE per row.\n\nGPT-5.1 INSTRUCTIONS:\n1. Get column names from db_get_schema first\n2. Use your knowledge to fill ALL columns (web search if needed for obscure data)\n3. Call db_add_row with BOTH block_id AND row_data\n4. row_data keys must EXACTLY match column names from schema\n5. Provide values for EVERY column, even if estimated\n\nEXAMPLE for planets table:\nColumns: [\"Name\", \"Type\", \"Mass (Earth=1)\", \"Radius (km)\", \"Moons\"]\nCall: db_add_row({\n  block_id: \"abc123\",\n  row_data: {\n    \"Name\": \"Jupiter\",\n    \"Type\": \"Gas Giant\",\n    \"Mass (Earth=1)\": \"317.8\",\n    \"Radius (km)\": \"69911\",\n    \"Moons\": \"95\"\n  }\n})\n\nDO NOT call without row_data! DO NOT leave columns empty!".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "block_id": {
                        "type": "string",
                        "description": "The database block ID from create_database or db_get_schema"
                    },
                    "row_data": {
                        "type": "object",
                        "description": "REQUIRED object with column_name: value pairs. Keys MUST exactly match column names from schema. Include a value for EVERY column. Example: {\"Name\": \"Jupiter\", \"Type\": \"Gas Giant\", \"Mass (Earth=1)\": \"317.8\", \"Radius (km)\": \"69911\", \"Moons\": \"95\"}",
...
async fn execute_db_query_rows(params: Value, db: &SqlitePool) -> Result<ToolResult, String> {
    ...
    let block: crate::db::Block = sqlx::query_as(
        "SELECT * FROM blocks WHERE id = ? AND type = 'database'"
    )
    .bind(block_id)
    .fetch_one(db)
    .await
...
async fn execute_db_update_rows(params: Value, db: &SqlitePool) -> Result<ToolResult, String> {
    let block_id = params.get("block_id")
        .and_then(|v| v.as_str())
        .ok_or("Missing block_id")?;
    ...
    let mut data: Value = serde_json::from_str(&block.data)
        .map_err(|e| format!("Invalid block data: {}", e))?;

    // Get columns for name-to-id mapping
    let columns = data.get("columns")
        .and_then(|v| v.as_array())
        .ok_or("No columns in database")?
        .clone();

    // Create name -> id mapping
    let mut col_name_to_id: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    for column in columns.iter() {
        let col_obj = column.as_object().ok_or("Invalid column format")?;
        let col_id = col_obj.get("id").and_then(|v| v.as_str()).ok_or("Column missing id")?;
        let col_name = col_obj.get("name").and_then(|v| v.as_str()).ok_or("Column missing name")?;
        col_name_to_id.insert(col_name.to_string(), col_id.to_string());
    }

    // Get rows
    let rows = data.get_mut("rows")
        .and_then(|v| v.as_array_mut())
        .ok_or("No rows array in database")?;

    let mut updated_count = 0;

    // Update matching rows
    for row in rows.iter_mut() {
        let row_obj = row.as_object_mut().ok_or("Invalid row format")?;
        let row_data = row_obj.get_mut("data").and_then(|v| v.as_object_mut()).ok_or("No row data")?;

        // Check if row matches filter
        let matches = filter.iter().all(|(col_name, expected_val)| {
            if let Some(col_id) = col_name_to_id.get(col_name) {
                row_data.get(col_id) == Some(expected_val)
            } else {
                false
            }
        });
        ...
        if matches {
            // Apply updates
            for (col_name, new_val) in updates.iter() {
                if let Some(col_id) = col_name_to_id.get(col_name) {
                    row_data.insert(col_id.to_string(), new_val.clone());
                }
            }
            updated_count += 1;
        }
    ...
    let updated_data = serde_json::to_string(&data)
        .map_err(|e| format!("Failed to serialize data: {}", e))?;

    sqlx::query("UPDATE blocks SET data = ? WHERE id = ?")
        .bind(&updated_data)
        .bind(block_id)
        .execute(db)
        .await
    ...
    Ok(ToolResult {
        success: true,
        data: json!({
            "message": format!("Updated {} row(s)", updated_count),
...
async fn execute_db_add_row(params: Value, db: &SqlitePool) -> Result<ToolResult, String> {
    let block_id = params.get("block_id")
        .and_then(|v| v.as_str())
        .ok_or("Missing block_id")?;

    let row_data = params.get("row_data")
        .and_then(|v| v.as_object())
        .ok_or("Missing row_data")?;
    ...
    // Get columns to validate row data
    let mut columns = data.get("columns")
        .and_then(|v| v.as_array())
        .ok_or("No columns in database")?
        .clone();

    // Auto-fix: If any column is missing an ID, generate one and update the database
    let mut columns_fixed = false;
    for column in columns.iter_mut() {
        if let Some(col_obj) = column.as_object_mut() {
            if !col_obj.contains_key("id") || col_obj.get("id").and_then(|v| v.as_str()).map(|s| s.is_empty()).unwrap_or(true) {
                let new_id = uuid::Uuid::new_v4().to_string();
                eprintln!("⚠️ Auto-fixing: Column '{}' missing ID, generated: {}",
                    col_obj.get("name").and_then(|v| v.as_str()).unwrap_or("unknown"),
                    new_id);
                col_obj.insert("id".to_string(), json!(new_id));
                columns_fixed = true;
            }
        }
    }

    // If we fixed columns, update the database block
    if columns_fixed {
        data["columns"] = json!(columns);
        let fixed_data = serde_json::to_string(&data)
            .map_err(|e| format!("Failed to serialize fixed data: {}", e))?;
        ...
        sqlx::query("UPDATE blocks SET data = ? WHERE id = ?")
            ...
            .execute(db)
            .await
            .map_err(|e| format!("Failed to save fixed columns: {}", e))?;

        eprintln!("✅ Auto-fixed database columns saved to block {}", block_id);
    }

    // Build row with column IDs
    let mut new_row_data: serde_json::Map<String, Value> = serde_json::Map::new();

    for column in columns.iter() {
        let col_obj = column.as_object().ok_or("Invalid column format")?;
        let col_id = col_obj.get("id").and_then(|v| v.as_str()).ok_or("Column missing id after fix attempt")?;
        let col_name = col_obj.get("name").and_then(|v| v.as_str()).ok_or("Column missing name")?;

        // Check if row_data has value for this column (by name)
        if let Some(value) = row_data.get(col_name) {
            new_row_data.insert(col_id.to_string(), value.clone());
        }
    }
    ...
    // Get existing rows and add new row
    let row_count = {
        let rows = data.get_mut("rows")
            .and_then(|v| v.as_array_mut())
            .ok_or("No rows array in database")?;
...
async fn execute_db_delete_rows(params: Value, db: &SqlitePool) -> Result<ToolResult, String> {
    let block_id = params.get("block_id")
        .and_then(|v| v.as_str())
        .ok_or("Missing block_id")?;
    ...
    // Get rows and perform deletion
    let (deleted_count, remaining_count) = {
        let rows = data.get_mut("rows")
            .and_then(|v| v.as_array_mut())
            .ok_or("No rows array in database")?;
        ...
        rows.retain(|row| {
            ...
            let row_data = row_obj.and_then(|r| r.get("data")).and_then(|v| v.as_object());
            ...
            if let Some(row_data) = row_data {
                ...
                !matches // Keep rows that DON'T match (delete matching rows)
            } else {
                true // Keep invalid rows (don't delete)
            }
        });

        let deleted = initial_count - rows.len();
    ...
    Ok(ToolResult {
        success: true,
        data: json!({
            "message": format!("Deleted {} row(s)", deleted_count),
...
async fn execute_artifact_parse_structure(params: Value, db: &SqlitePool) -> Result<ToolResult, String> {
    ...
    // Get artifact block
    let block: crate::db::Block = sqlx::query_as(
        "SELECT * FROM blocks WHERE id = ? AND type = 'artifact'"
    )
    .bind(block_id)
    .fetch_one(db)
    .await
