...
// ============================================================================
// ADVANCED AI TOOLS - Phase 1 & 2 Implementation
// ============================================================================
...
pub async fn ai_chat_with_auto_tools(
    ...
    let openai_tools: Vec<crate::ai::Tool> = tools.iter().map(|t| {
        crate::ai::Tool {
    ...
    if let Some(nid) = note_id {
        if let Ok(context) = ai_get_note_context(nid.clone(), db.clone()).await {
            enhanced_messages.insert(0, crate::ai::Message {
                role: "system".to_string(),
                content: format!(
    ...
    let (mut response, mut tool_calls) = manager.agent_service.chat(
    ...
    while !tool_calls.is_empty() && iteration < max_iterations {
        ...
        for tool_call in &tool_calls {
            let tool_name = tool_call.function.get("name")
                .and_then(|v| v.as_str())
                .ok_or("Missing tool name")?;

            let arguments = tool_call.function.get("arguments")
                .cloned()
                .unwrap_or(serde_json::json!({}));

            // Parse arguments if it's a string
            let params = if let Some(args_str) = arguments.as_str() {
            ...
            match result {
                ...
                Err(e) => {
                    enhanced_messages.push(crate::ai::Message {
                        role: "function".to_string(),
                        content: format!("Error executing {}: {}", tool_name, e),
                    });
                }
            }
        }
        ...
        let (new_response, new_tool_calls) = manager.agent_service.chat(
            enhanced_messages.clone(),
            Some(openai_tools.clone())
        ).await?;
